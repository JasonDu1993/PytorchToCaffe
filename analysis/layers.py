        ...
        def transfer_input(_input):
            if isinstance(_input, Base):
                ...
                assert isinstance(input,
                                  Blob), 'The input of layer %s is not Blob, please use nn_tools.P.blob.Blob as input' % name
            return _input
    ...
    def __init__(self, input, kernel_size, num_out, stride=1, pad=0, name='sliding', ceil=False, transpose=False):
        ...
        for i in range(self.input.dim - 2):
            if not transpose:
                if not ceil:
                    outs.append(
                        np.floor(float(self.input[2 + i] + self.pad[i] * 2 - self.kernel_size[i]) / self.stride[i]) + 1)
                else:
                    outs.append(
                        np.ceil(float(self.input[2 + i] + self.pad[i] * 2 - self.kernel_size[i]) / self.stride[i]) + 1)
            else:
                # transpose
